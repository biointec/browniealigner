...
void ST_FindNumLeafs(NODE *node, int* numNodes)
{
        NODE *curr = NULL;
        ...
        curr = node->sons;
        while (curr != 0) {
                ST_FindNumLeafs(curr, numNodes);
                curr = curr->right_sibling;
        }
}

void ST_FindLeafPositions(NODE *node, int* currIndex, int* positions)
{
        NODE *curr = NULL;

        if (node->sons == 0) {
                positions[*currIndex] = node->path_position;
                (*currIndex)++;
                return;
        }
        ...
        curr = node->sons;
        while (curr != 0) {
                ST_FindLeafPositions(curr, currIndex, positions);
                curr = curr->right_sibling;
        }
}

int ST_FindAllSubstrings(
                      /* The suffix array */
                      SUFFIX_TREE*    tree,
                      /* The substring to find */
                      char*  W,
                      ...
                      int** startingPositions)
{
   ...
   while(node!=0)
   {
      ...
      if(j == P)
      {
         ...
         ST_FindNumLeafs(node, &numLeafs);

         *startingPositions = (int*)malloc(sizeof(int)*numLeafs);

         ST_FindLeafPositions(node, &currIndex, *startingPositions);
         return numLeafs;
      }
      else if(k > node_label_end)
         /* Current edge is found to match, continue to next edge */
